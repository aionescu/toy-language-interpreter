...
module Language.TL.TypeChecker(isSubtypeOf, typeCheck) where
...
instance Show TypeError where
  show te = "Type error: " ++ go te ++ "."
    where
      ...
      go (UndeclaredVar ident)
        | isUpper $ head ident = "The type \"" ++ ident ++ "\" is not in scope"
        | otherwise = "The variable \"" ++ ident ++ "\" is not in scope"
      ...
      go CanOnlyAddNumbers = "Both operands of the addition operation must be numbers"
...
typeCheck' (NumLit n) = pure (Num, NumLit n)
typeCheck' (BoolLit b) = pure (Bool, BoolLit b)
typeCheck' (StrLit s) = pure (Str, StrLit s)
...
typeCheck' (Close e) = do
  (tf, ef) <- typeCheck' e
  tf `mustBe` File
  pure (unit, Close ef)

typeCheck' (NewRef e) = do
  (t, e') <- typeCheck' e
  pure (Ref t, NewRef e')

typeCheck' (RefAssign lhs rhs) = do
  (tl, el) <- typeCheck' lhs
  tl' <- unwrapRef tl
  ...
  pure (unit, RefAssign el (upCast tl' tr er))
